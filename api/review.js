...
async function fetchText(url, timeout = 6500) {
  ...
  try {
    const r = await fetch(url, {
      headers: {
        'user-agent': 'Mozilla/5.0 (compatible; SimpleReviewMaker/1.1)',
...
function inferCategory(place, menu, researchText) {
  ...
  const defs = [
    ...
    { key: 'japanese', label: '일식 음식점', words: ['일식','초밥','스시','사시미','돈까스','돈카츠','라멘','우동','소바','규동','덮밥','이자카야'] },
  ...
  // 메뉴에 음식 키워드가 있으면 검색문서의 카페 단어 수가 조금 많아도 음식점으로 우선한다.
...
async function diningCodeResearch(place) {
  ...
  return {
    ...
    url: profileUrl,
    text,
    relevance: relevanceScore(place, text.slice(0, 12000))
  };
}
...
function researchedSentences(features, categoryKey) {
  ...
  if (categoryKey === 'cafe') {
    if (keys.has('ocean')) pool.push('바다 전망이 좋다는 후기가 많은 곳이라 뷰까지 함께 즐기기 좋았어요.');
    if (keys.has('space')) pool.push('공간이 넓고 층별로 둘러보기 좋다는 이야기가 많은 편이라 여유롭게 머물기 좋았어요.');
    if (keys.has('interior')) pool.push('인테리어나 포토존 이야기가 자주 보여서 공간 구경하는 재미도 있는 곳이었어요.');
    if (keys.has('dessert')) pool.push('디저트 메뉴에 대한 언급도 많은 편이라 커피와 같이 즐기기 좋았어요.');
    if (keys.has('parking')) pool.push('주차가 편하다는 후기도 많아 차로 방문하기 괜찮은 곳이었어요.');
    if (keys.has('quiet')) pool.push('편하게 쉬기 좋다는 평이 많아 여유롭게 머물기 좋았어요.');
  } else {
    if (keys.has('flavor')) pool.push('음식 맛이 좋다는 후기가 많이 보이는 곳이라 전체적으로 만족스럽게 식사했어요.');
    if (keys.has('portion')) pool.push('양이 넉넉하다는 이야기도 많아서 든든하게 먹기 좋았어요.');
    if (keys.has('side')) pool.push('반찬이나 메뉴 구성이 괜찮다는 평이 많아 한 끼 식사로 만족스러웠어요.');
    if (keys.has('meat')) pool.push('고기 상태나 식감에 대한 좋은 평이 많아 고기 메뉴 좋아하면 만족하기 좋은 곳이었어요.');
    if (keys.has('soup')) pool.push('국물이나 육수 맛을 좋게 보는 후기가 많아 뜨끈하게 먹기 좋았어요.');
    if (keys.has('fresh')) pool.push('재료가 신선하다는 평도 자주 보여서 음식 전반이 깔끔한 느낌이었어요.');
    if (keys.has('family')) pool.push('가족끼리 식사하기 좋다는 후기도 많아 모임 장소로도 괜찮아 보여요.');
    if (keys.has('solo')) pool.push('혼자 식사하기 편하다는 이야기도 있어 가볍게 한 끼 먹기 좋았어요.');
    if (keys.has('parking')) pool.push('주차가 편하다는 후기도 많아 차로 방문하기 괜찮았어요.');
    if (keys.has('clean')) pool.push('매장이 깔끔하다는 평이 많아 편하게 식사하기 좋았어요.');
  }
...
function buildReview({ place, menu, note, length, tone, emoji, features, category }) {
  ...
  const researchPool = researchedSentences(features || [], category?.key || 'restaurant');
  if (researchPool.length && (length !== '아주 짧게' || !note)) parts.push(pick(researchPool));
...
module.exports = async function handler(req, res) {
  ...
  // 요청마다 현재 입력값만 사용한다. 브라우저의 최근 리뷰/localStorage는 이 API로 전달되지 않는다.
  ...
  let research = {
    researched: false,
    category: inferCategory(place, menu, ''),
    features: [],
    sources: []
  };
